...
class ElementIndicator:
    ...
    def draw(self, figure: Figure, lines=False) -> None:
        ...
        info(
            f"Element indicator for: {self.element.symbol}-{self.element.atomic_number}: label_artists: {len(self.label_artists)}"
        )
    ...
    def _set_label(self, *args, **kwargs):
        ...
        if self._is_plotted.get():
            self.marker_artist.set_visible(True)
        else:
            self.marker_artist.set_visible(False)
    ...
    def set_x_scale(self, figure):
        ...
        visible_labels = sorted(
            [
                l
                for l in self.label_artists
                if x_min < ax.transData.transform(l.artist.get_position())[0] < x_max
            ],
        ...
        if not visible_labels:
            return
        label_x_positions = [
            ax.transData.transform(l.artist.get_position())[0] for l in visible_labels
        ]
        visible = []
        last_visible = None
