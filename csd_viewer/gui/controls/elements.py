...
class _CustomElementManager(tk.Frame):
    ...
    def add_element(self, element: Element):
        ...
        self._custom_elements.append(
            _CustomElement((row, column), element, element_frame)
        )
...
class ElementButtons(ttk.Frame):
    def __init__(
        ...
        **kwargs,
        ...
        self.element_visibility = {
            ...
            for e in persistent_elements + variable_elements
        }
        self._plot = plot
    ...
    def create_widgets(self):
        ...
        lbOptions = tk.Label(
            self, text="Display options", font=self._subtitle_font, justify="left"
        )
        ...
        lbPersistent = tk.Label(
            frElement, text="Persistent", font=self._subtitle_font, justify="center"
        )
        ...
        lbVariable = tk.Label(
            frElement, text="Variable", font=self._subtitle_font, justify="center"
        )
        lbVariable.grid(column=2, row=0, sticky="NW", columnspan=2)
        for i, element in enumerate(
            sorted(self._variable_elements, key=lambda e: e.atomic_number)
        ):
            text = f"{element.symbol}-{element.atomic_mass}"
            button = ttk.Checkbutton(
                frElement,
        ...
        pad = 5
        pady = 5
        lbSymbol = tk.Label(frCustom, text="Symbol:").grid(
            column=0, row=0, padx=pad, pady=pady
        )
        entSymbol = ttk.Entry(frCustom, textvariable=self.varSymbol, width=5).grid(
            column=1, row=0, padx=pad, pady=pady
        )
        lbAtomicMass = tk.Label(frCustom, text="Mass:").grid(
            column=2, row=0, padx=pad, pady=pady
        )
        entAtomicMass = ttk.Entry(frCustom, textvariable=self.varMass, width=5).grid(
            column=3, row=0, padx=pad, pady=pady
        )
        lbAtomicNumber = tk.Label(frCustom, text="At. no.:").grid(
            column=4, row=0, padx=pad, pady=pady
        )
        entAtomicNumber = ttk.Entry(
            frCustom, textvariable=self.varNumber, width=5
        ).grid(column=5, row=0, padx=pad, pady=pady)
        btAddCustom = ttk.Button(
            frCustom, text="Add", bootstyle="outline", command=self.add_custom_element
        ).grid(column=6, row=0, padx=pad, pady=pady)

        frCustom.pack(side="top")
        self._custom_elements.pack(side="top")
        btnRemoveCustomElements = ttk.Button(
            self,
            text="Clear all",
            bootstyle="outline",
            command=self._custom_elements.remove_all_elements,
        ).pack(side="top")
    ...
    def get_element(self) -> Element:
        ...
        if not mass.isdigit():
            ...
            for element in self._persistent_elements + self._variable_elements:
                if (
                    int(number) == element.atomic_number
                    and int(mass) == element.atomic_mass
            ...
            for element in self._custom_elements._custom_elements:
                if (
                    int(number) == element.element.atomic_number
        ...
        if warning:
            if not messagebox.askyesno(
                "Warning", warning + ". Are you sure you want to add it?"
            ):
                return None
